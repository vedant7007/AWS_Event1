...
const year0Questions = [
  // ===== CTO — Cloud Architect (Q1–Q8) =====
  {
    ...
    difficulty: 'easy',
    scoringRubric: { full: 10, partial: 0, incorrect: -5 }
  },
  {
...
async function seedDatabase() {
  try {
    console.log('🌱 Starting Year 1 (Round 1) seed...');
    ...
    console.log('🗑️  Clearing existing Year 0 (Round 1) questions...');
    const deleted = await Question.deleteMany({ year: 0 });
    console.log(`✓ Removed ${deleted.deletedCount} old questions`);

    console.log(`📚 Inserting ${year0Questions.length} Year 1 questions...`);
    const inserted = await Question.insertMany(year0Questions);
    ...
    console.log('\n📊 Year 1 Breakdown:');
    byRole.forEach(r => console.log(`   ${r._id.toUpperCase()}: ${r.count} questions`));

    const total = await Question.countDocuments();
    console.log(`\n   Total questions in DB: ${total}`);
    console.log('\n✅ Year 1 seeding complete!');
    process.exit(0);
  } catch (error) {
